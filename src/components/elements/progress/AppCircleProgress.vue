...
<template>
  <div ref="elem" v-bind="wrapAttr" class="ol-circular-progressbar">
    <svg v-bind="svgAttr">
    ...
    <span class="current-counter">
      <slot></slot>
      <span v-if="!hasDefaultSlot && showPercent">
        {{ currentPercent }} {{ unit }}
      </span>
    </span>
  </div>
...
.ol-circular-progressbar {
  position: relative;
}

.ol-circular-progressbar .current-counter {
  position: absolute;
  top: 50%;
  left: 50%;
