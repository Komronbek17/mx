...
<template>
  <div class="layout-container">
    <div v-if="votes && votes.length" class="vote-class">
      ...
      <vote-answers :vote="votes[activeVote-1]" :active-index="activeVote" v-model="answerIds[activeVote-1].values"/>

      <div class="vote-buttons">
        <button :disabled="activeVote<=1" @click="backQuestion" class="vote-btn vote-back">
          <img src="@/assets/icons/arrow-left.svg" alt="">
          <p>
            Назад
          </p>
        </button>
        <button v-if="!lastQuestion" :disabled="!checkValid" @click="nextQuestion"
    ...
    <modal-dialog :model-value="activePrizeModal" @close-modal="closePrizeModal">
      <template #header>
...
.vote-buttons {
  ...
  .vote-back {
    ...
    p {
      @include text-gradient(linear-gradient(145.27deg, #313331 -5.05%, #181716 105.01%));
    }
  }
