...
export function useInfo () {
    ...
    async function checkTelegramAuth () {
        try {
            ...
            const {data} = await api.telegram.checkUserApi({telegram_id: tg.initDataUnsafe?.user?.id})
    ...
    async function getDailyInfo () {
        await api.info.getDailyInfoApi()
            .then(({data}) => {
    ...
    async function getPremiumInfo () {
        await api.info.getPremiumInfoApi()
            .then(({data}) => {
