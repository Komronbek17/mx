...
const light = {
  ...
  "--gf-blue-gradient-01": "linear-gradient(180deg, #00BBF9 0%, #00A3FF 100%)",
  "--gf-blue-gradient-02":
    "linear-gradient(107.32deg, #4ADAFF -22.08%, #0062CA 122.03%)",
...
function setStyleVariable({ property, variable }) {
  document.documentElement.style.setProperty(property, variable);
}

function isDarkMode() {
  return WebAppController.webApp.colorScheme === "dark";
}

function setThemeVariables() {
  const rootVariables = isDarkMode()
    ? keys(theme, "dark")
    : keys(theme, "light");
...
export function themeComposable() {
  return {
    setThemeVariables,
  };
}
